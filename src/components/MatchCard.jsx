  ...
  return (
    <div className={`${cardClass} p-5 fade-in`}>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-1.5" style={{ fontSize: "0.66rem", color: "#64748B" }}>
          <Calendar size={10}/>
      ...
      <div className="flex items-center justify-between gap-3 py-2">
        ...
        <div className="flex flex-col items-center gap-2.5 flex-1 min-w-0">
          <div
            ...
            style={isFinal ? { borderColor: "rgba(244,197,66,0.4)", width: "52px", height: "52px" } : {}}
          >
            <FlagIcon teamName={match.team1} size={isFinal ? 42 : 36}/>
          </div>
          <span
            className="text-center font-semibold leading-tight text-white"
            style={{ fontSize: "0.82rem", wordBreak: "break-word", maxWidth: "90px" }}
          >
            {match.team1}
          </span>
        </div>
        ...
        <div className="flex flex-col items-center gap-2 flex-shrink-0">
          ...
          {isFinal && (
            <span style={{ fontSize: "1.3rem", filter: "drop-shadow(0 0 10px rgba(244,197,66,0.7))" }}>🏆</span>
          )}
        </div>
      ...
      {/* Footer */}
      <div className="flex items-center justify-between gap-2 flex-wrap" style={{ fontSize: "0.68rem" }}>
        <div className="flex items-center gap-2">
          <Clock size={11} style={{ color: "#22C55E", flexShrink: 0 }}/>
          <span className="font-bold" style={{ color: "#22C55E", fontSize: "0.7rem" }}>
            {formatTime(match.time)}
          </span>
          <span className={period.className}>{period.emoji} {period.label.split(" ")[0]}</span>
        </div>
        <div className="flex items-center gap-1.5 text-right" style={{ color: "#475569" }}>
